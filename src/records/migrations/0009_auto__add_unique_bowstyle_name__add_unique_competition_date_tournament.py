...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding unique constraint on 'Bowstyle', fields ['name']
        db.create_unique('records_bowstyle', ['name'])

        # Adding unique constraint on 'Competition', fields ['date', 'tournament']
        db.create_unique('records_competition', ['date', 'tournament_id'])

        # Adding unique constraint on 'Club', fields ['name']
        db.create_unique('records_club', ['name'])

        # Adding unique constraint on 'Entry', fields ['archer', 'shot_round']
        db.create_unique('records_entry', ['archer_id', 'shot_round_id'])

        # Adding unique constraint on 'Archer', fields ['club', 'name']
        db.create_unique('records_archer', ['club_id', 'name'])

        # Adding unique constraint on 'Arrow', fields ['entry', 'arrow_of_round', 'subround']
        db.create_unique('records_arrow', ['entry_id', 'arrow_of_round', 'subround_id'])

        # Adding unique constraint on 'Tournament', fields ['full_name']
        db.create_unique('records_tournament', ['full_name'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Tournament', fields ['full_name']
        db.delete_unique('records_tournament', ['full_name'])

        # Removing unique constraint on 'Arrow', fields ['entry', 'arrow_of_round', 'subround']
        db.delete_unique('records_arrow', ['entry_id', 'arrow_of_round', 'subround_id'])

        # Removing unique constraint on 'Archer', fields ['club', 'name']
        db.delete_unique('records_archer', ['club_id', 'name'])

        # Removing unique constraint on 'Entry', fields ['archer', 'shot_round']
        db.delete_unique('records_entry', ['archer_id', 'shot_round_id'])

        # Removing unique constraint on 'Club', fields ['name']
        db.delete_unique('records_club', ['name'])

        # Removing unique constraint on 'Competition', fields ['date', 'tournament']
        db.delete_unique('records_competition', ['date', 'tournament_id'])

        # Removing unique constraint on 'Bowstyle', fields ['name']
        db.delete_unique('records_bowstyle', ['name'])
