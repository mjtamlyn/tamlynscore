...
urlpatterns = patterns('entries.views',
    ...
    url(r'^(?P<slug>[\w-]+)/entries/add/$', views.EntryAdd.as_view(), name='entry_add'),
    ...
    url(r'^(?P<slug>[\w-]+)/score-sheets/session/(?P<session_id>\d+)/$', views.SessionScoreSheetsPdf.as_view(), name='session_score_sheets_pdf'),
